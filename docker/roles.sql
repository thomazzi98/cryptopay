...
-- most likely thing in the deployment to be compromised. If it held the same credentials as the API,
-- compromising it would mean reading every merchant's payments and every API key digest.
--
...
--
-- Applied AFTER the migrations, not as a database init hook. The init hook runs at database creation,
-- when none of these tables exist yet, and every GRANT then fails with "relation does not exist" —
-- which the image reports as a warning and then carries on, leaving a database with no roles and a
-- deployment that looks fine until something tries to connect.
--
-- Idempotent, because it runs on every start.

\set ON_ERROR_STOP on

-- PostgreSQL has no CREATE ROLE IF NOT EXISTS, and a plain CREATE ROLE fails the whole script on the
-- second run.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'cryptopay_api') THEN
    CREATE ROLE cryptopay_api LOGIN PASSWORD 'cryptopay_api_local';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'cryptopay_chain_worker') THEN
